...
/* No arquivo sqlcommands.c */

void op_update(Lista *tuples, inf_query *queryInfo) {
    char *tableName = queryInfo->tabela;
    char *colName = queryInfo->update_column_name;
    char *newValue = queryInfo->update_value;
    ...
    if (!esquema) {
        printf("ERROR: Could not load schema for table \"%s\".\n", tableName);
        return;
    }
    ...
    tp_table *tempEsquema = esquema;
    while (tempEsquema != NULL) {
        if (strcasecmp(tempEsquema->nome, colName) == 0) {
            colIndex = qtdCampos;
        }
        qtdCampos++;
        tempEsquema = tempEsquema->next;
    }
    ...
    char path[200];
    sprintf(path, "%s/%s", connected.db_directory, objeto.nArquivo);
    
    FILE *fp = fopen(path, "r+b");
    if (!fp) {
        printf("ERROR: Could not open table file.\n");
        return;
    }

    /* --- CÁLCULO DO PULO (HEADER + PADDING) --- */
    /* Se o primeiro campo for INT ou DOUBLE, o compilador alinha para 4 bytes.
       Então temos: 1 byte validade + 3 bytes padding = 4 bytes de header.
       Se for CHAR ou STRING, não tem padding, então é só 1 byte. */
    int jump = 1;
    if (esquema->tipo == 'I' || esquema->tipo == 'D') {
        jump = 4;
    }

    char **tupleBuffer = (char **)malloc(qtdCampos * sizeof(char *));
    Nodo *ptr = tuples->prim;

    while (ptr != NULL) {
        tupla *t = (tupla *)ptr->inf;
        unsigned int offset = t->offset;

        // printf("DEBUG: Offset: %u | Jump: %d\n", offset, jump); // Debug

        /* Pular Header Corretamente (Valid + Padding) */
        fseek(fp, offset + jump, SEEK_SET);

        /* 2. Ler tupla */
        tempEsquema = esquema; 
        int i = 0;
        while (tempEsquema != NULL) {
            if (tempEsquema->tipo == 'I') { 
                int val;
                fread(&val, sizeof(int), 1, fp);
                tupleBuffer[i] = malloc(12);
                sprintf(tupleBuffer[i], "%d", val);
            } 
            else if (tempEsquema->tipo == 'D') { 
                double val;
                fread(&val, sizeof(double), 1, fp);
                tupleBuffer[i] = malloc(30);
                sprintf(tupleBuffer[i], "%lf", val);
            } 
            else if (tempEsquema->tipo == 'C') { 
                char val;
                fread(&val, sizeof(char), 1, fp);
                tupleBuffer[i] = malloc(2);
                tupleBuffer[i][0] = val;
                tupleBuffer[i][1] = '\0';
            } 
            else if (tempEsquema->tipo == 'S') { 
                int size = tempEsquema->tam; 
                tupleBuffer[i] = malloc(size + 1);
                fread(tupleBuffer[i], sizeof(char), size, fp);
                tupleBuffer[i][size] = '\0';
            }
            tempEsquema = tempEsquema->next;
            i++;
        }
        ...
        /* 3. Modificar Valor */
        free(tupleBuffer[colIndex]);
        
        tempEsquema = esquema;
        for(int k=0; k < colIndex; k++) tempEsquema = tempEsquema->next;

        if (tempEsquema->tipo == 'S') {
            int maxSize = tempEsquema->tam;
            tupleBuffer[colIndex] = malloc(maxSize + 1);
            strncpy(tupleBuffer[colIndex], newValue, maxSize);
        } else {
            tupleBuffer[colIndex] = strdup(newValue);
        }

        /* 4. Escrever de volta (Usando o mesmo jump) */
        fseek(fp, offset + jump, SEEK_SET);
        ...
        while (tempEsquema != NULL) {
            if (tempEsquema->tipo == 'I') {
                int val = atoi(tupleBuffer[i]);
                fwrite(&val, sizeof(int), 1, fp);
            } 
            else if (tempEsquema->tipo == 'D') {
                double val = atof(tupleBuffer[i]);
                fwrite(&val, sizeof(double), 1, fp);
            } 
            else if (tempEsquema->tipo == 'C') {
                char val = tupleBuffer[i][0];
                fwrite(&val, sizeof(char), 1, fp);
            } 
            else if (tempEsquema->tipo == 'S') {
                int size = tempEsquema->tam;
                char *tempBuffer = calloc(size, sizeof(char));
                strncpy(tempBuffer, tupleBuffer[i], size);
                fwrite(tempBuffer, sizeof(char), size, fp);
                free(tempBuffer);
            }
            free(tupleBuffer[i]); 
            tempEsquema = tempEsquema->next;
