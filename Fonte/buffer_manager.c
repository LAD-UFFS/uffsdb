...
// Inicializa o Buffer Manager e o Buffer Pool
...
// Destrói o Buffer Manager, descarregando todas as páginas sujas
...
int BM_LoadPage(BufferManager* bm, const char* filename, unsigned int page_id, tp_buffer* target_frame) {
    ...
    if (target_frame->id != (unsigned int)INVALID_PAGE_ID && target_frame->db) {
        if (Disk_WritePageByName(target_frame->filename, target_frame->id, target_frame, bm->pool->page_size) != 0) {
...
// Obtém uma página do Buffer Pool pelo par (filename, page_id)
...
// Grava uma página suja específica no disco
int BM_FlushPage(BufferManager* bm, const char* filename, unsigned int page_id) {
    if (bm == NULL || filename == NULL) return -1;
    tp_buffer* frame = BP_FindPageByFile(bm->pool, page_id, filename);
    if (frame != NULL) {
        if (frame->db) {
            if (Disk_WritePageByName(frame->filename, frame->id, frame, bm->pool->page_size) != 0) {
...
// Grava todas as páginas sujas do pool no disco
