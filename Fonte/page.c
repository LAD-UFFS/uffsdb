...
/**
 * @brief Extrai uma única tupla de uma posição específica dentro dos dados de um tp_buffer.
 *        Retorna NULL se a tupla estiver deletada ou o offset for inválido.
 */
...
/**
 * @brief Extrai todas as tuplas válidas de um frame.
 */
...
/**
 * @brief Escreve uma tupla nos dados de um tp_buffer em uma posição específica.
 *
 * valorCampo deve conter os bytes binários do valor (não string para I/D/C).
 * O código de INSERT em sqlcommands.c já monta bufferTuple com bytes binários
...
int PAGE_SetTupla(tp_buffer *page_buffer, unsigned int offset,
    ...
    *current_ptr = 0;
    ...
    for (int ic = 0; ic < objeto.qtdCampos; ic++) {
        if (nova_tupla->column[ic].valorCampo == COLUNA_NULL ||
            nova_tupla->column[ic].valorCampo == NULL) {
            nullos[ic] = 1;
        }
    }
    current_ptr += objeto.qtdCampos;
