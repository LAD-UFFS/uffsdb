...
// Imprime os registros de um PageResult
...
/* ----------------------------------------------------------------------------------------------
    Objetivo: Insere uma nova tupla na tabela filename
    Retorno: Percorre as páginas existentes procurando espaço. Se não houver,
...
int RM_DeleteRecord(BufferManager* bm, const char* filename, tp_table *s, struct fs_objects objeto,
...
int RM_UpdateRecord(BufferManager* bm, const char* filename, tp_table *s, struct fs_objects objeto,
                    unsigned int page_id, unsigned int offset, const tupla *tupla_atualizada) {
...
/* ----------------------------------------------------------------------------------------------
    Objetivo:  Itera pelas páginas do arquivo filename e coleta todas as tuplas válidas
    Retorno: Para quando encontra uma página vazia (position == 0) ou quando BM_GetPage falha (fim lógico do arquivo).
   ---------------------------------------------------------------------------------------------*/
        ...
        if (pr != NULL) {
            for (int i = 0; i < pr->nrec; i++) {
                ...
                for (unsigned int ci = 0; ci < cp->ncols; ci++) {
                    ...
                    if (pr->tuplas[i].column[ci].valorCampo != COLUNA_NULL && pr->tuplas[i].column[ci].valorCampo != NULL) {
