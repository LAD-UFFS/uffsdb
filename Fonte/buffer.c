...
// Variável global do BufferManager 
BufferManager *global_buffer_manager = NULL;

// Inicializa o BufferManager global e retorna um frame livre do pool inicializado com zeros para a página id
// O parâmetro filename indica o arquivo da tabela que será associado ao frame
tp_buffer* initBuffer(unsigned int id) {
    ...
    // Retorna um frame livre do pool para que possa escrever diretamente em frame->data
    ...
    if (frame == NULL) {
        ...
        if (frame->db && frame->filename[0] != '\0') {
            Disk_WritePageByName(frame->filename, frame->id, frame, global_buffer_manager->pool->page_size);
...
column* excluirTuplaBuffer(tp_buffer *buffer, tp_table *campos, struct fs_objects objeto, int page_id, int nTupla) {
    ...
    int result = RM_DeleteRecord(global_buffer_manager, filepath, campos, objeto, (unsigned int)page_id, (unsigned int)target_offset);
    if (result != 0) {
        return ERRO_PARAMETRO;
    }
    return NULL;
...
// Grava o frame `buffer` no disco usando o filename armazenado no próprio frame. 
// Se o filename estiver vazio (frame criado por initBuffer antes de um INSERT), o objeto é usado para montar o caminho.
