    ...
    for (page = 0; page < PAGES; page++)
    {
        ...
        for (int j = 0; j < pagina->nrec; j++)
        {
            for (int i = 0; i < objeto.qtdCampos; i++)
                if (pagina)
                { // Não necessita verificar se pagina[j].column[i].nomeCampo é NULL, pois se pagina foi alocada o nomeCampo não será NULL.
...
int finalizaInsert(char *nome, column *c, int tamTupla)
{
    ...
    if (objeto.lastBuffer == -1)
    {
        ...
        buffer = bm_novaPaginaNoBuffer(objeto.cod, 0, directory); // criamos o bloco 0 da tabela diretamente no buffer (e não gravamos logo em seguida no disco)
        objeto.lastBuffer = 0;
...
void adcResultado(Lista *resultado, tupla *tuple, int *indiceProj, int qtdColunasProj)
{
    ...
    for (uint i = 0; i < tuple->ncols; i++)
    {
        ...
        if (c->valorCampo == (void *)COLUNA_NULL)
            nw->token = COLUNA_NULL;
    ...
    for (int i = 0; i < qtdColunasProj; i++)
        adcNodo(tuplaRes, tuplaRes->ult, listNw[indiceProj[i]]);
}
...
char identificaTipoValor(char *valor)
{
    ...
    if (strlen(valor) == 1 && !isdigit(valor[0]) && valor[0] != '-')
        return 'C';
...
void createTable(rc_insert *t)
{
    ...
    for (i = 0; i < t->N; i++)
    {
        ...
        if (tupleSize > BLOCK_SIZE)
        { // validação para não exceder o limite da tupla
