...
// INVALID_PAGE_ID = -23 em macros.h
// Como tp_buffer.id é unsigned int, o valor -23 em complemento de dois

// Inicializa o Buffer Pool com a quantidade de frames e tamanho de página especificados
BufferPool* BP_Init(unsigned int num_frames, unsigned int page_size) {
    // Usa malloc diretamente
...
// Procura uma página específica no Buffer Pool pelo par (page_id, filename)
...
// Procura uma página pelo par (page_id, filename)
...
// Encontra um frame livre (id == INVALID_PAGE_ID) no Buffer Pool
...
// Seleciona um frame vítima para substituição usando LRU
...
// Atualiza o tempo de último acesso de um frame (para LRU)
