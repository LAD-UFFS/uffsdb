...
int drawline(tp_pagina *buffpoll, tp_table *s, struct fs_objects objeto, int p, int num_page)
{
    ...
    // antes aux recebendo tamTuplaSemByteControle que retorna o tamanho total da tupla na tabela, e aux iniciava com a multiplicação de p (posição da tupla na página) por tamTuplaSemByteControle, desconsiderando o byte de controle (posição de bufferTuple), por isso ele começava a ler um byte antes, que era o header sobre a validade da coluna ou não
    int tamTupla = tamTuplaSemByteControle(s, objeto) + 1 + objeto.qtdCampos; // agora, antes de atribuir aux, o tamTupla recebe o tamanho total da tupla por meio de tamTuplaSemByteControle (tamanho da tupla total - só considerando as colunas) + 1 (para o byte de controle) + a quantidade de colunas da tabela (para os bytes de controle de cada coluna)

    int num_reg = objeto.qtdCampos; // refere-se a qtd de colunas da tabela
    int aux = p * tamTupla;         // multiplica o a posição da tupla na página pelo tamanho total da tupla (tamTupla) para obter a posição inicial da tupla na página
    aux += 1 + objeto.qtdCampos;    // como não quero ler as flags de controle se o elemento é válido ou não (pq na impressão deixa o campo vazio), pulo elas (+1) e pulo objeto.qtdCampos pq quero iniciar já no valor da primeira coluna da tupla
    int *pos_ini = &aux;            // pega o endereço de início e percorre a tupla
    int count, pos_aux, bit_pos;
    union c_double cd;
    union c_int ci;
    int x = 0;
    // exemplo pra entnder melhor: na página está assim----> flag | id | nome | id (dado) | nome (dado)
    //                                                         1B   1B     1B      4B          10B
    // tamTuplaSemByte = 14B (nome(dado) + id(dado))
    // tamTupla = 17B (flag, id, nome, id (dado), nome (dado))
    // num_reg = 2
