...
void resetQuery() {
    if(getMode() == OP_SELECT || getMode() == OP_DELETE || getMode() == OP_UPDATE) { // <--- Adicione OP_UPDATE
        if(QUERY.tabela) {
            free(QUERY.tabela);
            QUERY.tabela = NULL;
        }
        ...
        if(QUERY.update_column_name) {
            free(QUERY.update_column_name);
            QUERY.update_column_name = NULL;
        }
        if(QUERY.update_value) {
            free(QUERY.update_value);
            QUERY.update_value = NULL;
        }
    }
}
                ...
                if (!connected.conn_active) {
                    ...
                    switch(GLOBAL_PARSER.mode) {
                        ...
                        case OP_DELETE:
                            ...
                            break;
                        case OP_UPDATE:
                            // 1. Encontra as linhas (REAPROVEITAMENTO!)
                            resultado = handleTableOperation(&QUERY, 'u'); // 'u' para update
                            
                            if (resultado) {
                                // 2. Chama a nova função de lógica de update
                                op_update(resultado, &QUERY); 
                                
                                // 3. Imprime quantas linhas foram afetadas
                                printf("UPDATE %d\n", resultado->tam);
                                
                                // 4. Limpa a lista de resultados
...
/* Pode ser similar a setColumnInsert, mas mais simples */
void setUpdateColumn(char **nome) {
    /* strdup aloca memória e copia a string */
    QUERY.update_column_name = strdup(*nome); 
}

void setUpdateValue(char *nome, char type) {
    /* Esta parte precisa de cuidado, especialmente para strings.
       Você pode copiar a lógica da sua função setValueInsert() 
       para remover as aspas simples das strings. */
       
    int i;
    if (type == 'S') {
        QUERY.update_value = malloc(sizeof(char)*(strlen(nome)-1));
        for (i = 1; i < strlen(nome)-1; i++) {
            QUERY.update_value[i-1] = nome[i];
        }
        QUERY.update_value[strlen(nome)-2] = '\0';
    } else {
        QUERY.update_value = strdup(nome);
    }
    
    QUERY.update_value_type = type;
}